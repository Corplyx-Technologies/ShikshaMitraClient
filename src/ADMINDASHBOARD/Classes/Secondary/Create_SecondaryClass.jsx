...
function Create_PrimaryClass() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true)
      const response = axios.post('https://tiny-tan-wombat-shoe.cyclic.app/api/v1/adminRoute/createClass', formData, {
  ...
  const handleDelete = async (className) => {
    try {
      const re = await axios.get(`https://tiny-tan-wombat-shoe.cyclic.app/api/v1/adminRoute/getAllClass?className=${className}`, {withCredentials: true,
      headers: {
        Authorization: `Bearer ${authToken}`,
      },});

      // Make an API request to delete the row from the server
      const response = await axios.delete(`https://tiny-tan-wombat-shoe.cyclic.app/api/v1/adminRoute/deleteClass?_id=${re.data.classList[0]._id}`, { withCredentials: true,
      headers: {
        Authorization: `Bearer ${authToken}`,
      }, });
