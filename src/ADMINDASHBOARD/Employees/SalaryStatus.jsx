...
const EmployeeSalaryStatus = () => {
  ...
  useEffect(() => {
    axios
      .get(`https://precious-pink-nightgown.cyclic.app/api/v1/adminRoute/getAllEmployees?email=${email}`, {
  ...
  useEffect(() => {
    ...
    if (employeeId) {
      axios
        .get(`https://precious-pink-nightgown.cyclic.app/api/v1/employee/getPaymentHistory?employeeId=${employeeId}`, {
          withCredentials: true,
      headers: {
        Authorization: `Bearer ${authToken}`,
      },
  ...
  const handleSubmit = () => {
    ...
    const apiUrl = `https://precious-pink-nightgown.cyclic.app/api/v1/employee/salaryPay`;
    axios
