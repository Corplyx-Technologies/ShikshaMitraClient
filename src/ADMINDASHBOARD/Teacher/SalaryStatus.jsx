...
const SalaryStatus = () => {
  ...
  useEffect(() => {
    axios
      .get(`https://tiny-tan-wombat-shoe.cyclic.app/api/v1/adminRoute/getTeachers?email=${email}`, {
  ...
  useEffect(() => {
    if (teacherId) {
      axios
        .get(`https://tiny-tan-wombat-shoe.cyclic.app/api/v1/teacher/getPaymentHistory?teacherId=${teacherId}`, {
          withCredentials: true,
      headers: {
        Authorization: `Bearer ${authToken}`,
      },
  ...
  const handleSubmit = () => {
    ...
    const apiUrl = `https://tiny-tan-wombat-shoe.cyclic.app/api/v1/teacher/salaryPay`;
    axios
