...
const PieChart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('https://grumpy-plum-dalmatian.cyclic.app/api/v1/adminRoute/getAllStudents', {
      ...
      headers: {
        ...
        if (Array.isArray(response.data.allStudent)) {
          console.log('Students', response.data.allStudent);
          // Filter students by gender
          const boysCount = response.data.allStudent.filter(student => student.gender === 'Male').length;
